...
World::World() {
  ...
  gameWorld_->system<Tags::D2>("2D").each([this](Tags::D2& d2) {
    ...
    frame_.data.push_back({glm::vec2(400.0f, 400.0f), glm::vec2(800.0f, 800.0f),
                           0.0f, 1.0f, glm::vec4(0.0f)});
  ...
  gameWorld_->system<Tags::D2, Components::Position>("Position")
      .each([this](const Tags::D2 d2, const Components::Position& pos) {
        const auto renderer = sl_->GetService<Renderers::IRenderer>();
        renderer->GetFrame()->data[d2.index - 1].pos =
